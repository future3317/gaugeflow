...
最新多轮诊断的可执行同步版见
[`stage_e_generated_lattice_diagnosis_sync_2026_07_24.md`](stage_e_generated_lattice_diagnosis_sync_2026_07_24.md)。
该文档固定了 counts、shape residual、lattice-coordinate counterfactual、partial/MASK
exposure、v1 freeze 与 A-v2 generated-state contract 的因果边界；后续接手不得只凭聊天
上下文重启已排除的 E-v1 调参路线。
...
随后已经完成更宽的 32-source real replay cache 和早期 dose 诊断。新 cache 位于
`/home/workspace/lrh/DATA/T2C-Flow/evaluations/generated_state_replay_32_real_v1/`，
包含 32 个真实 source、4 个 roles、共 128 条 entries；`selection_seed=6101`，
manifest SHA-256 为
`f59f58545bc1dab62664fad39b14806c0ef42e85f3d786c3cbaee78f131e4909`，并通过
773 个 forbidden source IDs 的 overlap 检查。训练合约审计通过：四个 role 的 terminal
element/lattice/coordinate 梯度组均非零，clean retention loss ratio to max generated 为
`0.3621880364977982`。32-source 20-step smoke 也通过，确认参数实际更新且
clean-retention max ratio 为 `0.7123302917464417`。

32-source 34M dose 结果表明，问题不是简单容量不足，而是 generated-state replay 在仍偏窄的
support 上过度优化或更新剂量过强。100-step EMA 把 replay role losses 全部压低，同时
free NN-W1 仅从 `0.538407` 轻微漂移到 `0.545142`，volume-W1 从 `0.333797` 改善到
...
随后补跑了更细的 10/15/20-step EMA 剂量点以定位是否存在交叉窗口。10-step checkpoint
`/home/workspace/lrh/DATA/T2C-Flow/runs/generated_state_replay_correctness_34m_64src_10_v1/checkpoint_step_00000010.pt`
的 SHA-256 为 `e49d01c1a67d7b3fb64e090703ea1e55bf4ce16c10b18a6f1459ec9fd55e8ee3`；
15-step 为 `d2fc6d1bb1c7b9088d329e84901f9b3c08c1a42fe7f3e9654dafac3f046da6ca`；
20-step 为 `2d0921d7da0c5ed3364fc9641dc553b0c03c3c217ea6b7abbaae7c96f31a6c26`。
三者训练审计均通过，所有 final role terminal gradient groups 非零，forbidden source ID 检查仍覆盖
773 个 IDs。

val128 全剂量 selector
`/home/workspace/lrh/DATA/T2C-Flow/evaluations/generated_state_replay_64src_checkpoint_selection_val128_v4.json`
仍给出 `status=no_eligible_checkpoint`。按 steps `10/15/20/25/50`，val128 的
NN-W1 delta 为 `+0.002923/+0.010864/+0.011899/+0.014134/+0.021105`，volume-W1 delta 为
`+0.000934/+0.000786/+0.001154/+0.001539/+0.004166`，hard validity 全部不退化。
10-step 的 generated replay losses 降低但 clean_clean total loss 轻微上升 `+0.000272`，因此不满足
all-role loss 下降；15-step 起 all-role total losses 全部下降，但 volume-W1 仍严格为正。
这说明当前 64-source 34M replay 不是缺少一个简单早停点，而是 clean replay correctness 与
...
（SHA-256 `f003a264bf19f1b7a47683e873acab67d73bc591a5b3eae7966d5d747f12ef1d`）已经用同一
val128 record-mode evaluator 复核，并与临时 combo 逐项完全一致。官方结果保存在
...
all-role replay total losses 全部下降；free NN-W1 delta 为 `+0.0024332300859237765`；
volume-W1 delta 为 `-0.000040659420989408446`；volume bootstrap 95% CI 为
`[-0.0007411272712751895,+0.00030002156927976496]`；hard validity、exact composition、
finite-positive lattice、sampling failures 和 terminal masks 均不退化。

因此该 checkpoint 可以登记为 diagnostic trust-projection candidate，但仍不是 Stage-E pass 或
production checkpoint；还必须在独立 support/window 上验证后才能讨论后续训练。若该投影在独立窗口
不稳，则结论是 E-v1 replay update 方向过拟合当前 cache/window，Stage-E 继续 blocked。当前不应再
围绕 E-v1 adapter 做新的标量调参，也不应因为 GPU 空闲直接启动 58M/98M 或 Stage-F。

独立 support/window 复核随后已经完成：同一官方 projection checkpoint 分别在 32-source、64-source
和 128-source replay caches 上评估。三组 replay role total losses 全部下降。32-source 的
clean/generated_assignment/generated_lattice/generated_joint loss delta 分别为
`-0.0002186/-0.0010221/-0.0001602/-0.0036774`；64-source 为
`-0.00005245/-0.0019548/-0.0009167/-0.0034051`；128-source 为
`-0.0001791/-0.0018747/-0.0013530/-0.0034461`。三者使用相同 checkpoint、val128
reference 与 random streams，所以 free NN-W1/volume-W1 delta 相同，分别为
`+0.0024332300859237765` 和 `-0.000040659420989408446`，volume bootstrap CI 为
`[-0.0007411272712751895,+0.00030002156927976496]`；hard validity、exact composition、
finite-positive lattice、sampling failures 和 terminal masks 均不退化。

这足以结束 E-v1 标量调参并转入 GaugeFlow-base v2 重训计划，但不等于可以立即启动 58M/98M
长训。当前 `train_generated_state_replay_correctness.py` 只能从 34M Stage-C checkpoint 重建同形
backbone 并做 replay update，不能初始化和训练 58M/98M base model。下一步代码任务是实现冻结协议下的
A-v2 clean-plus-generated-state retraining runner：先支持 34M smoke，保留 provenance/forbidden-source/
exact-count/per-role-loss/paired-retention selector，再把 58M/98M 作为同预算 capacity competition
候选。GPU 空闲可以用于并行 smoke 和后续容量竞争，但不能绕过 34M A-v2 runner smoke。
