...
Stage-C-v2 已完成全部 50,000 次三流 continued-pretraining。三张 GPU 分别承担
LeMat structure、MatPES physical 与 Alex replay，并在每一步同步共享模型梯度；三个
stream cursor、optimizer、EMA 与随机状态共同进入原子 checkpoint。最终 50k 的 LeMat
macro loss/physical composite/NN-W1/volume-W1 为
`1.486348/0.250460/0.572337/0.067552`，hard validity 全部为一且零 sampling
failure。该完整轨迹证明真实 LeMat 迁移已经运行，不再只是 data-plane smoke。
...
20k/30k/40k/50k 均使用相同的 LeMat、physical 与 A1 面板。冻结的四目标
Pareto-minimax 规则排除被支配的 40k，并在 20k/30k/50k 最大归一化 regret
`1.0/0.539119/0.608750` 中选择 **Stage-C 30k**（global step 40,523，SHA-256
`8807877bbdcc61090a431dc5cd146ed62bf545b2a65425ff8bb16c8d0d317bf9`）作为
operational base；50k 保留为完整训练终点，而不是默认采用最后一步。
...
当前首个不可变 D cache 已完成：3,946 graphs、43,015 atoms，按 reduced-composition/
prototype 拆成 `3,173/398/375`，跨 split overlap 为零。piezoelectric 全覆盖，
dielectric/Born/Gamma 覆盖 3,943，严格 internal strain 覆盖 1,266；另有 2,893 条
审计合格的 JARVIS elastic targets，经 GPa engineering-Voigt → Kelvin → Cartesian
唯一转换后按 availability mask 接入。介电响应存在强重尾，不能用普通 RMS
把典型样本压缩数百倍，也不能把潜在软模记录当坏数据删除。正式数值边界采用 train-only
isotropic-location、robust scalar scale 与可逆 O(3)-协变 radial-asinh chart。

FlowMimic 启发只以轻量方式进入 D：完整 Cartesian tensor loss 保留，额外比较六方向
icosahedral response-field auxiliary 权重 `0` 与 `0.1`。D0 使用相同 Stage-C 30k EMA、
sample order、augmentation RNG 和单 seed 跑 2,000 paired updates；只有 probe error
...
E0 的首选优化是 common-noise orbit mimic：同一结构、同一 diffusion clocks 和同一
随机噪声只替换 (e' = \rho_3(h)e)，然后在正确的 gauge reindex 后比较 typed outputs；
...
F0 使用 common-random-number paired rollouts：冻结 E 与可训练 F 接收同一 ξ 和同一
tensor orbit，以 (R(G_F(ξ,[e]))-R(G_E(ξ,[e]))) 形成低方差 paired advantage。
trust region 直接约束异质反向场：categorical 用 KL/JS，coordinates 用周期切空间距离，
lattice 用 log-volume/log-shape chart；conditional 项保护 E 的条件能力，null 项保护 C 的
无条件结构 prior。F0 必须等待 E 通过 target-swap 与自由样本分布分离。
