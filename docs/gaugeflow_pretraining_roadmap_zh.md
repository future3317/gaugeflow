...
| MatPES-PBE / r2SCAN | 本地计划约 433k / 386k 成功记录 | PES 表征、energy/force/stress 与 noisy-state distillation | functional/source 必须分层校准 |
...
A0 的 bounded supported-IID 子模块现已分别通过，但 production `joint` trainer/sampler
尚未调用 `p(C|N)` 与 orderless exact-count assignment，不能把子模块结果写成完整
product-space runtime 通过。先完成该接线及独立 closure Gate，才允许冻结 A1；这不等于
自由 parent law 已闭合，也不授权 tensor、RL 或物理计算。
