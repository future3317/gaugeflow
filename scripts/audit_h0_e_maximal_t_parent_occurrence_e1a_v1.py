"""Independently rebuild and audit the frozen H0-E-v2 E1a pilot."""
...
def _source_rows(material_ids: set[str], data_root: Path) -> dict[str, dict[str, object]]:
...
def audit(config: dict[str, Any], data_root: Path) -> dict[str, object]:
    ...
    ids = [str(value) for value in config["selection"]["material_ids"]]
    result_by_id = {str(value["material_id"]): value for value in results}
    decomposition_path = data_root / "processed/gaugeflow_h0_v4/parent_decomposition_pilot_v1/decompositions.parquet"
    decompositions = {
    ...
    e0_path = data_root / "processed/gaugeflow_h0_v5/maximal_subgroup_embeddings_v2/catalogue_records.json.gz"
    e0_records = load_gzip_json(e0_path)
    by_child: dict[int, list[dict[str, object]]] = defaultdict(list)
    for record in reversed(e0_records):
    ...
    for material_id in ids:
        try:
            ...
            rebuilt = search_maximal_t_parents(
                child,
                by_child[child.space_group],
            ...
            expected = {(value.embedding_key, value.parent_space_group): value for value in rebuilt}
            observed = {
                (str(value["embedding_key"]), int(value["parent_space_group"])): value for value in stored["candidates"]
            }
            ...
            for key, occurrence in expected.items():
                ...
                scalars_match = all(
                    np.isclose(float(left), float(right), atol=1e-12, rtol=0.0) for left, right in scalar_pairs
                )
                numeric_matches += arrays_match and scalars_match
                strict_parent_matches += occurrence.parent_space_group == int(saved["parent_space_group"])
                finite_occurrences += all(np.isfinite(float(value)) for pair in scalar_pairs for value in pair)
    ...
    checks = {
        "results_hash_matches_manifest": sha256_file(results_path) == manifest["results_sha256"],
        "e0_hash_matches_protocol": sha256_file(e0_path) == config["dependencies"]["e0_records_sha256"],
        "v1_hash_matches_protocol": sha256_file(decomposition_path)
        == config["dependencies"]["v1_decompositions_sha256"],
        "selected_and_result_ids_exact": len(ids) == 64 and len(set(ids)) == 64 and set(result_by_id) == set(ids),
        ...
        "strict_parent_identification_reproduced": strict_parent_matches == rebuilt_occurrences,
        "all_rebuilt_occurrences_finite": finite_occurrences == rebuilt_occurrences,
    ...
    return {
        ...
        "decision": (
            "independent_audit_verified_frozen_E1a_result"
            if all(checks.values())
            else "independent_audit_failed_E1a_result_untrusted"
        ),
    }
