...
def _load_panel(
    ...
    *,
    ...
    preprocessed_cache: Path | None = None,
):
    data = protocol["data"]
    dataset = PiezoCrystalDataset(
        train_csv or _resolve(root, data["train_csv"]),
        split_manifest=split_manifest or _resolve(root, data["split_manifest"]),
        split=data["split"],
        target_cache_dir=target_cache_dir or _resolve(root, data["target_cache_dir"]),
