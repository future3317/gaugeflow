...
ordering；action-node alignment 与 primitive lattice 则完全有效。这个接口缺口现已
修复：geometry-complete v2 对 454/454 carrier 重建闭包全部通过，最大 periodic
alignment error 为 `4.61e-14 A`，target 字段与 carrier 字段结构隔离。

后续 geometry-aware 表示审计也已完成。仅 unary geometry signature 可解决
`47.36%`；对剩余 239 个冲突类，完整 transferable two-point descriptor 解决
`87.87%`，总体 mean target ceiling 为 `0.93933`，且 relabel、`GL(3,Z)` 与
target-orbit containment 均零失败。这证明 geometry 是必要信息，但不把静态 pair
histogram 当作最终概率律。

因此实现了 target-independent uniform reveal order 的 remaining-count law：

```text
p(A_zr=k | partial,C,B) proportional to n_k_remaining * exp(logit_zr,k)
```

它在每一步消耗一个真实 count，无 terminal mask、Hungarian repair 或 CIF-row order；
完整 assignment marginal 可在小系统上用 subset DP 审计，parent quotient 只累加
unique orbit labelings。Q0 已通过：normalization error `4.44e-16`、subset-DP/reference
error `1.04e-17`、FP64/FP32 equivariance `1.11e-15/4.77e-7`、residual-stabilizer
error `2.98e-7`、exact-count sampling `1.0`、BF16 cosine `0.999974`。RTX 4090 的
