...
第一次 clean production 集成随后按冻结标准失败并已撤回。它的结构、BF16 一致性和
性能均正常：参数量 `4,479,161`、无旧 readout key、输出余弦 `0.99623`、梯度余弦
`0.98573`、`1066.85 graphs/s / 506.24 MiB`。但 target-free output-energy 的
FP32/BF16 梯度 norm 为 `373.27/407.55`，均超过 `100`。因此问题是 carrier 接入
fractional score 后的绝对 Jacobian 尺度，而不是精度方向崩溃。该实现只保留在 Git
`e25f432`；active production 恢复原 combined head，尚不允许 target fit 或训练。
